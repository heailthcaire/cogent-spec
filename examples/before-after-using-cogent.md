...
### Where The Cogent Text Goes

The Cogent text below belongs in a reusable skill file.

Suggested location:

```text
skills/product-opportunity-review.cogent.md
```

or, for this repository:

```text
examples/product-opportunity-review.cogent.md
```

The frontmatter and methods define the workflow. They are not the user's prompt. The user's long, messy prompt becomes the value passed into `[[rough_idea]]`.

In a native Cogent runtime, the workflow would be called like this:

```text
CALL METHOD product-opportunity-review::main[[rough_idea]]
```

In today's LLM tools, paste or attach the skill first, then provide the rough idea and ask the model to execute `main`.

Example current-use prompt:

```text
Use the product-opportunity-review Cogent skill below.

Execute main with this rough_idea:
[paste the user's messy prompt here]
```
